...
class Entry (object):
  """
  Su dung port de xet xem port nao forward traffic out
  Su dung MAC de dap lai ARP replies
  Su dung timeout de neu mot entry lau hon ARP_TIMEOUT, chung ta
  se flood ARP reuquest thay vi co gang de tra loi no
  """
...
class l3_switch (EventMixin):
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    #ba thong so cai them vao detect DDoS
    global set_Timer
    global defendDDOS
    global blockPort
    timerSet =False
    ...
    #Nhung cac ham vao controller de detect DDoS
    def preventing(): #ham de ngan chan tan cong
      global diction
      global set_Timer #Cai dat thoi gian
      if not set_Timer:
        set_Timer =True
    ...
    def _timer_func (): #Ham block IP
      ...
      diction={}
    #Ket thuc phan nhung vao controller
    ...
    if isinstance(packet.next, ipv4):
      ...
      #ent_obj thu thap du lieu goi tin va tinh toan entropy
      ent_obj.collectStats(event.parsed.next.dstip)
      print ("Entropy : ",str(ent_obj.value)) #Tinh toan entropy
      if ent_obj.value <1.0: #Neu entropy < 1 thi prevent DDoS
        preventing()
        ...
        timerSet=False
     #Ket thuc phan tinh toan entropy
